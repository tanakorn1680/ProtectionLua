/**
 * Loader Generator — Server Only
 *
 * สร้าง Loader.lua สำหรับ Roblox
 * Loader มีเฉพาะ: protection_id, api_endpoint
 * ไม่มี Key, Secret, หรือ Source ใดๆ
 *
 * Loader Flow:
 *   1. ผู้ใช้ตั้งค่า LICENSE และ DEVICE_ID
 *   2. POST /api/runtime/session → ได้ session_token
 *   3. POST /api/runtime/payload + session_token → ได้ XOR-encrypted source
 *   4. XOR decode ด้วย session_token → plain source bytes
 *   5. loadstring(source)()
 *
 * Security notes:
 *   - session_token อายุ 5 นาที, single-use (revoked หลัง fetch payload)
 *   - Loader ไม่รู้ encryption key ของ payload ที่เก็บใน server
 *   - static analysis ของ Loader ไม่เจอ source
 *   - ดัก HTTP → ได้ XOR data ที่ไม่มี key (key คือ session_token ที่ expire แล้ว)
 */

export interface LoaderConfig {
  protectionId: string
  originalFilename: string
  apiEndpoint: string   // e.g. "https://your-app.vercel.app"
  createdAt: string
}

export function generateLoader(config: LoaderConfig): string {
  const { protectionId, originalFilename, apiEndpoint, createdAt } = config

  // Lua XOR decode function — counterpart ของ prepareDeliveryXor ใน engine.ts
  // ใช้ SHA-256 ที่ implement ใน pure Lua (พอร์ตจาก public domain)
  // Roblox มี game:HttpGet และ loadstring
  return `-- ============================================================
-- Loader: ${originalFilename}
-- Protection: ${protectionId}
-- Generated: ${createdAt}
-- ============================================================
-- คำเตือน: ไฟล์นี้สร้างโดยอัตโนมัติ ไม่ควรแก้ไข
-- ============================================================

local PROTECTION_ID = "${protectionId}"
local API_ENDPOINT  = "${apiEndpoint}"

-- ตั้งค่าโดยผู้ใช้
local LICENSE_KEY = "YOUR-LICENSE-KEY-HERE"
local DEVICE_ID   = tostring(game.PlaceId) .. "-" .. tostring(game.JobId):sub(1,8)

-- ============================================================
-- SHA-256 implementation (pure Lua, public domain)
-- ใช้สำหรับ XOR keystream derivation
-- ============================================================
local sha256
do
  local band,bxor,bnot,rshift,lshift=bit32.band,bit32.bxor,bit32.bnot,bit32.rshift,bit32.lshift
  local function u32(n) return n % 4294967296 end
  local K={0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,
    0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,
    0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,
    0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,0x06ca6351,0x14292967,
    0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,0x650a7354,0x766a0abb,0x81c2c92e,0x92722c85,
    0xa2bfe8a1,0xa81a664b,0xc24b8b70,0xc76c51a3,0xd192e819,0xd6990624,0xf40e3585,0x106aa070,
    0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,0x391c0cb3,0x4ed8aa4a,0x5b9cca4f,0x682e6ff3,
    0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2}
  local function rotr(x,n) return u32(rshift(x,n)+lshift(x,32-n)) end
  sha256=function(msg)
    local bits=0
    local s={0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19}
    msg=msg.."\x80"
    while #msg%64~=56 do msg=msg.."\x00" end
    bits=#msg*8-8
    msg=msg..string.char(0,0,0,0,math.floor(bits/0x1000000)%256,math.floor(bits/0x10000)%256,math.floor(bits/0x100)%256,bits%256)
    for i=1,#msg,64 do
      local w={}
      for j=1,16 do
        local o=(i-1)+((j-1)*4)
        w[j]=lshift(msg:byte(o+1),24)+lshift(msg:byte(o+2),16)+lshift(msg:byte(o+3),8)+msg:byte(o+4)
        w[j]=u32(w[j])
      end
      for j=17,64 do
        local s0=bxor(rotr(w[j-15],7),rotr(w[j-15],18),rshift(w[j-15],3))
        local s1=bxor(rotr(w[j-2],17),rotr(w[j-2],19),rshift(w[j-2],10))
        w[j]=u32(w[j-16]+s0+w[j-7]+s1)
      end
      local a,b,c,d,e,f,g,h=table.unpack(s)
      for j=1,64 do
        local S1=bxor(rotr(e,6),rotr(e,11),rotr(e,25))
        local ch=bxor(band(e,f),band(bnot(e),g))
        local tmp1=u32(h+S1+ch+K[j]+w[j])
        local S0=bxor(rotr(a,2),rotr(a,13),rotr(a,22))
        local maj=bxor(band(a,b),band(a,c),band(b,c))
        local tmp2=u32(S0+maj)
        h=g g=f f=e e=u32(d+tmp1) d=c c=b b=a a=u32(tmp1+tmp2)
      end
      s[1]=u32(s[1]+a) s[2]=u32(s[2]+b) s[3]=u32(s[3]+c) s[4]=u32(s[4]+d)
      s[5]=u32(s[5]+e) s[6]=u32(s[6]+f) s[7]=u32(s[7]+g) s[8]=u32(s[8]+h)
    end
    local r=""
    for _,v in ipairs(s) do
      r=r..string.char(rshift(v,24)%256,rshift(v,16)%256,rshift(v,8)%256,v%256)
    end
    return r
  end
end

-- ============================================================
-- Base64 decode
-- ============================================================
local b64chars="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"
local function b64decode(s)
  s=s:gsub("[^"..b64chars.."=]","")
  local r=""
  for i=1,#s,4 do
    local n=0
    local pad=0
    for j=0,3 do
      local c=s:sub(i+j,i+j)
      if c=="=" then pad=pad+1;n=n*64
      else n=n*64+(b64chars:find(c,1,true)-1) end
    end
    r=r..string.char(rshift(n,16)%256)
    if pad<2 then r=r..string.char(rshift(n,8)%256) end
    if pad<1 then r=r..string.char(n%256) end
  end
  return r
end

-- ============================================================
-- XOR keystream derive (matches server engine.ts deriveKeystream)
-- seed = session_token, length = len(data)
-- ============================================================
local function deriveKeystream(seed,length)
  local blocks={}
  local counter=0
  local total=0
  while total<length do
    local block=sha256(seed..string.char(counter%256,math.floor(counter/256)%256))
    table.insert(blocks,block)
    total=total+#block
    counter=counter+1
  end
  local ks=table.concat(blocks)
  return ks:sub(1,length)
end

-- ============================================================
-- XOR decode
-- ============================================================
local function xorDecode(data,key)
  local ks=deriveKeystream(key,#data)
  local out={}
  for i=1,#data do
    table.insert(out,string.char(bit32.bxor(data:byte(i),ks:byte(i))))
  end
  return table.concat(out)
end

-- ============================================================
-- HMAC-SHA256 short checksum verify (first 16 hex chars = 8 bytes)
-- ============================================================
local function verifyChecksum(source,token,expected)
  -- อ่าน 64 bytes ของ sha256(token..source) แบบง่าย
  -- ใช้ XOR fold เพื่อให้ length พอดี 8 bytes → compare hex
  local h=sha256(token..source)
  local hex=""
  for i=1,8 do hex=hex..string.format("%02x",h:byte(i)) end
  return hex==expected
end

-- ============================================================
-- HTTP helper
-- ============================================================
local HttpService=game:GetService("HttpService")
local function post(path,body)
  local ok,res=pcall(function()
    return HttpService:PostAsync(
      API_ENDPOINT..path,
      HttpService:JSONEncode(body),
      Enum.HttpContentType.ApplicationJson,
      false
    )
  end)
  if not ok then return nil,"http_error" end
  local ok2,data=pcall(function() return HttpService:JSONDecode(res) end)
  if not ok2 then return nil,"json_error" end
  return data,nil
end

-- ============================================================
-- Main Loader Logic
-- ============================================================
local function run()
  -- 1. Request session token
  local sessionData,err=post("/api/runtime/session",{
    protection_id=PROTECTION_ID,
    license=LICENSE_KEY,
    device_id=DEVICE_ID,
  })
  if not sessionData or not sessionData.success then
    local reason=(sessionData and sessionData.reason) or err or "unknown"
    error("[Loader] Session denied: "..reason)
    return
  end
  local sessionToken=sessionData.session_token

  -- 2. Request protected payload
  local payloadData,perr=post("/api/runtime/payload",{
    protection_id=PROTECTION_ID,
    session_token=sessionToken,
  })
  if not payloadData or not payloadData.success then
    local reason=(payloadData and payloadData.reason) or perr or "unknown"
    error("[Loader] Payload denied: "..reason)
    return
  end

  -- 3. Decode XOR data
  local rawB64=payloadData.data
  local checksum=payloadData.checksum
  if not rawB64 or not checksum then
    error("[Loader] Invalid payload format")
    return
  end

  local encrypted=b64decode(rawB64)
  local source=xorDecode(encrypted,sessionToken)

  -- 4. Verify checksum
  if not verifyChecksum(source,sessionToken,checksum) then
    error("[Loader] Integrity check failed — possible tampering")
    return
  end

  -- 5. Execute
  local fn,compileErr=loadstring(source)
  if not fn then
    error("[Loader] Compile error: "..(compileErr or "unknown"))
    return
  end
  fn()
end

run()
`
}
